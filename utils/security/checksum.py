...
from utils.common import error, success
...
@click.option(
    "--check", default=None, metavar="HASH", help="Checksum to verify against."
)
def file(path, algorithm, check):
    result = (
        hash_directory(path, algorithm)
        if os.path.isdir(path)
        else hash_file(path, algorithm)
    )
    if check:
